...
func TestPrimitive_codecov(_ *testing.T) {
	...
	primitiveCheckExplicitRead(1, &BERPacket{}, TLV{Class: 0, Tag: 2, Compound: true}, Options{Class: 0, Tag: 2})
}
...
func makeAdapter(shouldSucceed bool) adapter {
	return adapter{
...
func TestChainAdapters(t *testing.T) {
	...
	ad := chainAdapters([]adapter{
		makeAdapter(false), // will return error
		makeAdapter(true),  // will succeed
	})
